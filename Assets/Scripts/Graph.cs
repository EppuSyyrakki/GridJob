...
namespace GridJob
{
	public struct Graph
	{
		...
		public Graph(MapData data, bool log = false)
		{
			...
			for (int z = 0; z < data.size.z; z++)
			{
				for (int y = 0; y < data.size.y; y++)
				{
					...
					{
						tiles[i] = new Tile(x, y, z, Edge.None, Edge.None, TileType.Empty, i);
						i++;
					}
				}
			}
		...
		private Tile HandleClimb(Tile tile)
		{
			...
			if (GetIndex(tile + Tile.up, Data, out int aboveIndex)) 
			{
		...
		public static bool GetIndex(Tile t, MapData data, out int index)
        {
		...
		public static int GetIndex(Tile t, MapData data){ return GetIndex(t.data.x, t.data.y, t.data.z, data.size); }
		...
		public static bool HasTile(Tile t, MapData data) { return HasTile(t.data, data.size); }
